...
@implementation ShowDisplayOptionsView
...
- (void)viewDidLoad
{
    ...
    NSString *thePath = [[NSBundle mainBundle]  pathForResource:@"show-options" ofType:@"plist"];
	NSDictionary *dict = [[NSDictionary alloc] initWithContentsOfFile:thePath];
    self.optionsArray = [dict objectForKey:@"options"];
    TRACE("%s, count: %d\n", __func__, [self.optionsArray count]);
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPad) {
        self.contentSizeForViewInPopover = CGSizeMake(310.0, self.tableView.rowHeight*([self.optionsArray count]));
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    TRACE("%s, %s\n", __func__, [[self.optionsArray objectAtIndex:indexPath.row] UTF8String]);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [self.delegate changeDisplayView:indexPath.row];
}
