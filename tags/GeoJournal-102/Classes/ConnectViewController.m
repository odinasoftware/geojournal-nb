	...
	if (indexPath.section == MAIL_INDEX) {
		...
		if (testReachability() == false) {
			...
			if ([GeoSession sharedGeoSessionInstance].fbUID > 0 /*&& [GeoSession sharedGeoSessionInstance].fbUserName*/) {
				...
				[[GeoSession getFBAgent] loginToFacebookWithNotification:YES];
			}
		}
	}
	else {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (indexPath.section == MAIL_INDEX) {
		...
		else if ([GeoSession sharedGeoSessionInstance].fbUID && [GeoSession sharedGeoSessionInstance].gotExtendedPermission == YES) {
