...
- (void)reloadView
{
	...
	if (pictureLink) {
		...
		move = y - _imageRect.size.height;
		TRACE("%s, x: %f, y: %f, move: %f\n", __func__, x, y, move);
		
		TRACE("%s, y: %f\n", __func__, y);
		self.imageForJournal.frame = CGRectMake(_imageRect.origin.x+move_x/2.0, _imageRect.origin.y, x, y);
		...
		moveup = _imageRect.size.height - HEIGHT_WITHOUT_IMAGE;
