...
- (void)addToMapWithDirection:(MAP_DIRECTION_TYPE)direction upto:(NSInteger)size
{
	...
	for (i=_currentJournalIndex; i<[journalArray count]; ++i) {
		...
		TRACE("%s, %s, lg: %f, lt: %f\n", __func__, [j.title UTF8String], longitude, latitude);
...
- (MKAnnotationView *) mapView:(MKMapView *)view viewForAnnotation:(id <MKAnnotation>) annotation
{
	...
	if (mark.isCenter == YES) {
		...
		if (pictureLink) {
			NSData *data = [[NSData alloc] initWithContentsOfFile:getThumbnailFilename(pictureLink)];
		...
		TRACE("%s, %s\n", __func__, [mark.journalForLocation.title UTF8String]);
		annView.enabled = TRUE;
		annView.animatesDrop = YES;
		if (mark.active) {
...
- (UIButton *)getDetailDisclosureButtonType:(ItineraryMark*)mark
{
	...
	if (mark) {
		button = (UIButton*)[self._mapView viewWithTag:kViewTag];
		if (button == nil) {
